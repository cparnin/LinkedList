...
{
    if (!size) // if no size, insert front
        insertFront(number);
    else
    {
        ListNodePtr newNode = new ListNode(number, tail); // make new node
        testAllocation(newNode);
        size++; // list increased in size
        tail->next = newNode; // concatentate new node
        tail = newNode; // new tail
        newNode->next = NULL; // end of list is NULL
    }
} // List::insertBack()
...
{
    if (!iter.current) // if NULL, make front node
        insertFront(number);
    ListNodePtr node = new ListNode; // make new node
    testAllocation(node);
    iter = head; // iterate from head
    while (*iter != number) // find target number
        ++iter;
    if (iter == tail) // if we are entering after last node
        insertBack(number);
    node = ++iter; // set the new node to one after iterator
    iter.current = node; // iterator now points to new node
    ++size; // size increased
...
bool List::removeFront()
{
    if (!head) // if empty list
        return 0;
    if (head->next == NULL) // if only one node, just delete list
        makeEmpty();
    else
    {
    ...
    --size; // list decreased in size
    }
    return true; // if we removed the front node
} // List::removeFront()
